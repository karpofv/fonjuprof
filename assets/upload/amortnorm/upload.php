<?php 
	...
	if (!move_uploaded_file($tmp_archivo, $archivador)) {
		...
		//$handle = @fopen('../archivos/AMORT_NOR.csv',"r+");
        if ($handle) {
			$count==0;
			$insertar ="";
			$update = "";
			while (!feof($handle)) {
				...
				if ($count>0) {
					...
					if ($cons_asoc == 0) {
                        $insertar = "INSERT INTO amort_nor(PREST, ID, MONTO, VENC,TASA, PAGADA_EL, CANC, INTE, ABONO, PAGO, RESTA) VALUES ('$buffer[0]','$buffer[1]','$buffer[2]','$buffer[3]','$buffer[4]','$buffer[5]','$buffer[6]','$buffer[7]','$buffer[8]','$buffer[9]','$buffer[10]');";
                        $ejecutar = paraTodos::arrayejecutar($insertar);
						set_time_limit(1);
					} else {
						$update = "UPDATE amort_nor SET MONTO='$buffer[2]',TASA='$buffer[4]',VENC='$buffer[3]',PAGADA_EL='$buffer[5]',CANC='$buffer[6]',INTE='$buffer[7]',ABONO='$buffer[8]',PAGO='$buffer[9]',RESTA='$buffer[10]' WHERE PREST='$buffer[0]' and ID='$buffer[1]';";
                        $result = paraTodos::arrayejecutar($update);
						set_time_limit(1);
					}
				}
				$count=$count+1;
